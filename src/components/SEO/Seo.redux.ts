 ...
 * )
 */
export const sagaFetchSeoConf:any = createRoutine('saga/FETCH_SEO_CONF');

/**
 * STEP 3: Describe redux reducer, no saga related here
 */
export const reducer = handleActions({
  [sagaFetchSeoConf.SUCCESS]: (state, action) => ({
    ...state,
    ...action.payload,
    maxAge: moment().format('X')
  }),
  [sagaFetchSeoConf.FAILURE]: (state, action) => ({...state, error: action.payload.error}),
}, {...initialState});
